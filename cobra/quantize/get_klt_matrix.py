...
def get_llm_weight_klt(model, dataloader, num_samples=128):
    """
    基於 LLM 的權重分布估計每層的 KLT 矩陣。

    在原始版本中，這個函式透過 forward hook + dataloader 來觸發 stat_input_hook，
    但實際上 stat_input_hook 完全沒有使用 activations，而是直接對 m.weight 做統計。

    在 Cobra_1115 中，為了避免和 LLM forward 介面（input_ids / attention_mask / multimodal 等）
    發生不相容問題，這裡改為「直接掃描所有帶有 2D weight 的 module」，完全不依賴 dataloader：

        - 仍保留相同的特徵值分解 / gram_schmidt / K.T.float() 邏輯
        - 仍然跳過名字包含 "head" 的模組（和舊版行為一致）
        - 保留函式簽名 (model, dataloader, num_samples) 以維持相容性，dataloader/num_samples 不再使用
    """
    model.eval()
    device = next(model.parameters()).device
    weight_klt = {}
    ...
    def stat_tensor(name, tensor):
        ...
        # 轉到 float64 做協方差與特徵分解
        x = tensor.detach().to(device).reshape(-1, shape[-1]).double()
        # cov_matrix: [d, d]
        ...
        # 正交性檢查與修正
        KT_K = K @ K.T
        if KT_K[0, 0] > 0.99 and KT_K[0, 1].abs() < 0.0001:
            print(f"{name} weight is orthogonal")
        else:
            K = gram_schmidt(K)
            print(f"{name} weight is not orthogonal")

        # 與舊版保持一致：對權重使用 K.T
        weight_klt[name] = K.T.float()

    # 不再使用 forward hook，而是直接對所有帶有 2D weight 的 module 做統計
    for name, m in model.named_modules():
        # 與原實作一致，跳過名字含 "head" 的模組
        if "head" in name:
            continue
