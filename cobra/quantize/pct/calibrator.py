...
Responsibilities:
    ...
    - Convert (hi, lo) into affine quantization parameters (scale, zero_point),
      honoring the configured activation bitwidth.
...
def _set_quantizer_params(
    ...
    """
    Write calibrated parameters into a UniformAffineQuantizer instance.
    """
    device = _infer_module_device(quant)  # quant is also an nn.Module
    quant.change_n_bits(n_bits)

    # store as tensors so that later .to(device) / .cuda() calls behave as expected
    quant.scale = torch.tensor(float(scale), dtype=torch.float32, device=device)
    quant.round_zero_point = torch.tensor(int(zero_point), dtype=torch.float32, device=device)

    # Disable dynamic-observation behavior; this quantizer is now "frozen".
...
def _calibrate_single_module(
    ...
    *,
    ...
    Returns:
        True if any quantizer on this module was updated, False otherwise.
    """
    # Compute affine params once per module
    x_min = float(lo)
    x_max = float(hi)
    ...
    if hasattr(module, "act_quantizer") and isinstance(module.act_quantizer, UniformAffineQuantizer):
        _set_quantizer_params(module.act_quantizer, scale, zero, act_bits)
        applied = True

    # Pattern 2: QuantMatMul-style modules with quant_x1 / quant_x2
    for attr in ("quant_x1", "quant_x2"):
        q = getattr(module, attr, None)
        if isinstance(q, UniformAffineQuantizer):
            _set_quantizer_params(q, scale, zero, act_bits)
            applied = True
    ...
    *,
    ...
    Args:
        ...
        signed:
            ...
            features); otherwise as unsigned.
        include_targets:
    ...
    for record_key, rec in hi_lo_map.items():
        ...
        # Recompute scale/zero with the same helper we used when calibrating.
        x_min = lo
        x_max = hi
        scale, zero = compute_affine_params(x_min=x_min, x_max=x_max, bits=act_bits, signed=signed)
...
def calibrate_model_from_stats(
    ...
    *,
    ...
    Args:
        ...
        signed:
            Whether to treat activations as signed when computing scale/zp.
        include_targets:
