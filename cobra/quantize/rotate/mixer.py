...
class MixerHadamardRotationConfig:
    ...
    # Side gates
    in_transform_enabled: bool = True
    out_transform_enabled: bool = False

    # Per-side act-KLT gates
    in_act_klt_enabled: bool = False
    out_act_klt_enabled: bool = False

    # Optional explicit paths (override env/default)
    act_klt_in_path: Optional[str] = None
    act_klt_out_path: Optional[str] = None

    # If True: missing layer/block in act-klt payload is a hard error
    # (except layer0 OUT-KLT skip if enabled)
    act_klt_strict: bool = False

    # Required behavior: skip OUT-KLT on layer 0
...
def rotate_llm_mamba_mixers_hadamard_inplace(llm: nn.Module, *, cfg: MixerHadamardRotationConfig) -> Dict[str, object]:
    """
    Apply KLT+Hadamard to Mamba mixer out_proj, with mathematically closed-loop HK-IN and HK-OUT.
    ...
    in_tx = bool(getattr(cfg, "in_transform_enabled", True))
    out_tx = bool(getattr(cfg, "out_transform_enabled", False))
    in_klt_on = bool(getattr(cfg, "in_act_klt_enabled", False)) and in_tx
    out_klt_on = bool(getattr(cfg, "out_act_klt_enabled", False)) and out_tx
    ...
    if in_klt_on:
        in_path = resolve_mixer_act_klt_in_path(
            block_size=B,
            explicit_path=getattr(cfg, "act_klt_in_path", None),
        )
        if not in_path:
            msg = "[MixerHK] in_act_klt_enabled=1 but IN-KLT path cannot be resolved"
            if cfg.act_klt_strict:
                raise FileNotFoundError(msg)
            overwatch.warning(msg)
        else:
            k_in_layers = _load_klt_payload(str(in_path))
            overwatch.info(f"[MixerHK] Loaded IN act-KLT: path={str(in_path)!r}, layers={len(k_in_layers)}")

    if out_klt_on:
        out_path = resolve_mixer_act_klt_out_path(
            block_size=B,
            explicit_path=getattr(cfg, "act_klt_out_path", None),
        )
        if not out_path:
            msg = "[MixerHK] out_act_klt_enabled=1 but OUT-KLT path cannot be resolved"
            if cfg.act_klt_strict:
                raise FileNotFoundError(msg)
            overwatch.warning(msg)
        else:
            k_out_layers = _load_klt_payload(str(out_path))
            overwatch.info(f"[MixerHK] Loaded OUT act-KLT: path={str(out_path)!r}, layers={len(k_out_layers)}")
    ...
    # -----------------------------
    def _install_mixer_rotation_hooks(
        mod: nn.Module,
        *,
        block_size: int,
        rot_in_dim: int,
        ...
        do_in = bool(enable_in)
        do_out = bool(enable_out)

        if getattr(mod, "_cobra_mixer_rotation_hooks_installed", False):
            return
        ...
        if do_out:
            mod.register_forward_hook(_post_hook, with_kwargs=False)
        setattr(mod, "_cobra_mixer_rotation_hooks_installed", True)
        ...
        applied.append(
            {
                ...
                "layer": layer_idx,
                "mode": (
                    "hk_io" if (in_klt_on or out_klt_on)
                    else "h_io"
                ),
                "block": B,
                "in_transform": int(enable_in),
                "out_transform": int(enable_out),
                "rot_in_dim": rot_in_dim if enable_in else 0,
                "rot_out_dim": rot_out_dim if enable_out else 0,
                "has_k_in": int(torch.is_tensor(k_in)) if enable_in else 0,
                "has_k_out": int(torch.is_tensor(k_out)) if enable_out else 0,
                "in_act_klt": int(in_klt_on),
                "out_act_klt": int(out_klt_on),
